...
import {TimeInMs, TIME_NUMB, FilterType, DateFormat } from '../consts.js';

const humanizeEventDate = (date) => dayjs(date).format(DateFormat.MONTH_DAY);
const humanizeDataSetEventDate = (date) => dayjs(date).format(DateFormat.YEAR_MONTH_DAY);
const humanizeDataSetEventTime = (date) => dayjs(date).format(DateFormat.YEAR_MONTH_DAY_HOUR_MIN);
const humanizeDateTime = (date) => dayjs(date).format(DateFormat.HOUR_MIN);
const humanizeDateToCustomFormat = (date) => dayjs(date).format(DateFormat.DAY_MONTH_YEAR_HOUR_MIN);

const getFullTimeNumb = (numb) => numb < TIME_NUMB ? `0${numb}` : numb;

const getTimeDifferenceInMins = (start, end) => {
  const dateFrom = dayjs(start).format(DateFormat.YEAR_MONTH_DAY_HOUR_MIN);
